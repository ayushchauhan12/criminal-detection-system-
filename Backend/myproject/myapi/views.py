...
class CriminalImgDetectViews(APIView):
    ...
    def post(self, request):
        ...
        ussss=str(uuid.uuid4().int)
        print("hello")
        print(ussss)
        
        var = os.path.join(os.path.dirname(BASE_DIR), '') + r"\media\images_detected\image_" + ussss[0:3] + ".jpg"
        return_url = r"\images_detected\image_" + ussss[0:3] + ".jpg"
        test_img = cv.cvtColor(test_img, cv.COLOR_BGR2RGB)
